...
const Modal: React.FC<ModalProps> = ({ isOpen, onClose, title, children, icon }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl max-w-2xl w-full max-h-[80vh] flex flex-col animate-in zoom-in duration-200">
        ...
        <div className="p-4 border-t border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-700/50 rounded-b-xl flex justify-end">
...
export const PrivacyModal: React.FC<{ isOpen: boolean; onClose: () => void }> = ({ isOpen, onClose }) => (
  <Modal isOpen={isOpen} onClose={onClose} title="Privacy Policy" icon={<Shield size={24} />}>
    <h4 className="font-bold text-slate-800 dark:text-white">1. Local Processing</h4>
    ...
    <h4 className="font-bold text-slate-800 dark:text-white">2. Data Collection</h4>
    <p>We do not collect, store, or transmit your contact data. We do not use tracking cookies for advertising purposes.</p>

    <h4 className="font-bold text-slate-800 dark:text-white">3. Security</h4>
...
export const TermsModal: React.FC<{ isOpen: boolean; onClose: () => void }> = ({ isOpen, onClose }) => (
  <Modal isOpen={isOpen} onClose={onClose} title="Terms of Service" icon={<FileText size={24} />}>
    <h4 className="font-bold text-slate-800 dark:text-white">1. Usage</h4>
    ...
    <h4 className="font-bold text-slate-800 dark:text-white">2. Liability</h4>
    ...
    <h4 className="font-bold text-slate-800 dark:text-white">3. Updates</h4>
