    ...
    <div className="max-w-6xl mx-auto flex flex-col md:flex-row gap-8 relative">
        ...
        <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
           ...
           <div className="space-y-4">
             <div className="flex justify-between items-center py-2 border-b border-slate-100 dark:border-slate-700">
               <span className="text-slate-500 dark:text-slate-400">Total Contacts</span>
               <span className="font-semibold text-slate-900 dark:text-white">{contacts.length.toLocaleString()}</span>
             </div>
             <div className="flex justify-between items-center py-2 border-b border-slate-100 dark:border-slate-700">
               <span className="text-slate-500 dark:text-slate-400">Format</span>
               <span className="font-semibold text-slate-900 dark:text-white">{isVcfExport ? 'CSV -> VCF' : 'VCF -> CSV'}</span>
             </div>
           </div>

           {isVcfExport && (
             <div className="mt-6">
               <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">vCard Version</label>
               <select
                 className="w-full border border-slate-300 dark:border-slate-600 rounded-lg p-2.5 bg-slate-50 dark:bg-slate-700 dark:text-white"
           ...
           <button
             ...
             className="w-full mt-8 bg-blue-600 hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600 text-white py-3 rounded-lg font-semibold shadow-md shadow-blue-200 dark:shadow-none transition-all flex items-center justify-center space-x-2"
           >
             <Download size={20} />
           ...
           <button
             onClick={onBack}
             className="w-full mt-3 text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 py-2 text-sm font-medium flex items-center justify-center"
           >
      ...
      <div className="w-full md:w-2/3">
        <div className="flex justify-between items-end mb-4">
          <h3 className="text-lg font-bold text-slate-800 dark:text-white">Live Preview</h3>
          <div className="flex space-x-2">
            <button
              disabled={previewIndex === 0}
              onClick={() => setPreviewIndex(i => i - 1)}
              className="px-3 py-1 text-xs border border-slate-200 dark:border-slate-700 rounded bg-white dark:bg-slate-800 dark:text-white disabled:opacity-50"
            >
              Prev
            </button>
        ...
        {/* Mobile Preview Card */}
        <div className="bg-slate-800 dark:bg-slate-950 rounded-[2.5rem] p-4 max-w-sm mx-auto shadow-2xl border-4 border-slate-900 dark:border-slate-800 relative">
          {/* Notch */}
          <div className="absolute top-0 left-1/2 -translate-x-1/2 h-6 w-32 bg-slate-900 dark:bg-slate-800 rounded-b-xl z-10"></div>
        ...
        <p className="text-center text-slate-400 dark:text-slate-500 text-xs mt-4">Preview rendering may vary by device</p>
      </div>

      {/* QR Code Modal Overlay */}
      {showQR && (
        <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-sm z-50 rounded-xl flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-2xl max-w-sm w-full animate-in zoom-in duration-200">
            <div className="flex justify-between items-center mb-6">
              <h3 className="font-bold text-slate-900 dark:text-white">Scan to Save</h3>
              <button onClick={() => setShowQR(false)} className="text-slate-400 hover:text-slate-700 dark:hover:text-slate-200">
                <X size={20} />
              </button>
            </div>
