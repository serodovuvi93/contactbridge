      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
        <div className="p-6 border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 flex items-center justify-between">
           <div>
             ...
             <p className="text-sm text-slate-500 dark:text-slate-400">Divide your contacts into smaller manageable files.</p>
           </div>
           <div className="text-sm font-semibold bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300 px-3 py-1 rounded-full">
        ...
        <div className="p-8">
          ...
          <div className="grid grid-cols-2 gap-4 mb-8">
            <button
              ...
              className={`p-4 border-2 rounded-xl text-left transition-all ${splitMethod === 'by_count' ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20 ring-1 ring-blue-500' : 'border-slate-200 dark:border-slate-600 hover:border-blue-200 dark:hover:border-blue-500'}`}
            >
              <div className="flex items-center space-x-2 mb-2 text-slate-800 dark:text-white font-semibold">
                <FileText size={18} />
                <span>Contacts per File</span>
              </div>
              <p className="text-xs text-slate-500 dark:text-slate-400">Specify the maximum number of contacts in each file.</p>
            </button>

            <button
              onClick={() => setSplitMethod('by_files')}
              className={`p-4 border-2 rounded-xl text-left transition-all ${splitMethod === 'by_files' ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20 ring-1 ring-blue-500' : 'border-slate-200 dark:border-slate-600 hover:border-blue-200 dark:hover:border-blue-500'}`}
            >
               <div className="flex items-center space-x-2 mb-2 text-slate-800 dark:text-white font-semibold">
                <Layers size={18} />
                <span>Total Files</span>
              </div>
              <p className="text-xs text-slate-500 dark:text-slate-400">Specify exactly how many files you want to create.</p>
            </button>
          </div>
          ...
          <div className="mb-8">
             ...
             <input
                ...
                className="w-full text-2xl font-bold p-3 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-slate-900 dark:text-white dark:bg-slate-700"
             />
          </div>

          {/* Summary */}
          <div className="bg-slate-50 dark:bg-slate-700/30 rounded-lg p-4 border border-slate-200 dark:border-slate-700 flex items-center justify-between">
            <div className="text-sm text-slate-600 dark:text-slate-300">
               Result: <strong className="text-slate-900 dark:text-white">{fileCount} files</strong> with ~<strong className="text-slate-900 dark:text-white">{contactsPerFile}</strong> contacts each.
            </div>
          </div>
        ...
        <div className="p-6 border-t border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 flex justify-between items-center">
           <button
             onClick={onBack}
             className="text-slate-600 dark:text-slate-300 font-medium px-4 py-2 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-lg transition-colors flex items-center"
           >
